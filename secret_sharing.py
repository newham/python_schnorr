# from :https://github.com/frosty00/secret_sharing/blob/main/secret_sharing.py
#
...
WORDLIST_FILENAME = './data/bip-0039.txt'
...
def encode_secret(secret_indexes, shared_secrets: int, total_secrets: int):
    ...
    as_bits = ''.join(bin(index)[2:].rjust(SIZE, '0')
    ...
    for _ in range(total_secrets):
        ...
        y = sum(x ** (degree - i) * coefficient for i,
                coefficient in enumerate(equation))
...
def decode_secret(secret_words) -> str:
    ...
    for x, y in secrets_by_x.items():
        ...
        del copy[x]
        equation = equation + reduce(x_symbol, copy) * \
            sympy.invert(reduce(x, copy), prime_field) * y
    ...
    indexes = [int(bytes_solution[i:i + SIZE], base=2)
               for i in range(0, len(bytes_solution), SIZE)]
...
def encode_words(words: str) -> Point:
    ...
    bytes_solution = ''.join(bin(index)[2:].rjust(
        SIZE, '0') for index in indexes)
...
if __name__ == '__main__':
    ...
    print(
        f'encoded private key as {SHARED_SECRETS} of {len(encoded)} points on a polynomial')
