...
def ecc_times(x, y, n, a, p):
    for i in range(n):
        x, y = get_np(x, y, x, y, a, p)
    return x, y
...
def draw_graph(f, p):
    ...
    plt.savefig('./figs/scatter_%s.png' % (p))
    # plt.show()
    pass


def save_graph(path, a, b, p):
    """
    保存有限域的椭圆曲线散点
    """
    # x_y = []
...
def ecc_main():
    ...
    print("\nuser1解密得到明文：", end="")
    for charArr in c:
        decrypto_text_x, decrypto_text_y = get_ng(
            charArr[0], charArr[1], key, a, p)
